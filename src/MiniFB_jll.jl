...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import MiniFB_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("MiniFB_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "MiniFB_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"MiniFB", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "MiniFB_jll", "src", "MiniFB_jll.jl"))
        @info("MiniFB_ll dev'ed out to /depot/dev/MiniFB_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("b93afbca-07d8-5443-9979-bb4221978155"))
platforms = [Pkg.Artifacts.unpack_platform(e, "MiniFB", artifacts_toml) for e in artifacts["MiniFB"]]
...
if best_platform === nothing
    @debug("Unable to load MiniFB; unsupported platform $(triplet(platform_key_abi()))")
else
